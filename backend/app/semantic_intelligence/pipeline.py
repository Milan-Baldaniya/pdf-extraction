...
async def generate_chapter_intelligence(chapter_name: str, raw_markdown: str) -> Dict[str, Any]:
    """
    ...
    print("✂️  Phase 2: Slicing Chapter via LLM Quote-Matching...")
    slicer_result = slicer.analyze_and_slice(raw_markdown)
    ...
    for index, topic_data in enumerate(sliced_topics):
        ...
        try:
            # Execute the parallel Map-Reduce Swarm
            mega_concept_object = await swarm.process_topic_slice(content)
            
            # Since the user requested the TIO (TopicIntelligenceObject) layer:
            final_topic_obj = {
                "topic_name": title,
